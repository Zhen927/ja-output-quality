...
- `agents/`: レビュー担当4体（すべて Haiku 4.5。quick 用の reviewer は effort medium、full 用の3体は effort max）。レビューはセッションのモデルを引き継がない。
...
- レビューは、書き手と同じ系統の強いモデルより、別の弱いモデル（Haiku）が向く。自分の直訳を「馴染んだ文」として見過ごさないため。速くて安いので、full では effort を max にして回数より深さで補う。
